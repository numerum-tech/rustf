...
fn test_index_variable_in_loop() {
    ...
    assert!(
        html.contains(r#"data-bs-slide-to="0""#),
        "Should contain index 0"
    );
    assert!(
        html.contains(r#"data-bs-slide-to="1""#),
        "Should contain index 1"
    );
    assert!(
        html.contains(r#"data-bs-slide-to="2""#),
        "Should contain index 2"
    );

    // Check that the ternary expression works (index == 1 should be 'active')
    assert!(
        html.contains(r#"class="active""#),
        "Index 1 should have 'active' class"
    );
...
fn test_index_in_ternary_expression() {
    ...
    assert!(
        html.contains(r#"class="first""#),
        "Index 0 should have 'first' class"
    );
    assert!(
        html.contains(r#"class="second""#),
        "Index 1 should have 'second' class"
    );
    assert!(
        html.contains(r#"class="other""#),
        "Index 2 should have 'other' class"
    );
...
fn test_index_comparison_operations() {
    ...
    assert!(
        html.contains(r#"class="first""#),
        "Index 0 should match first condition"
    );
    assert!(
        html.contains(r#"class="second""#),
        "Index 1 should match second condition"
    );
    assert!(
        html.contains(r#"class="other""#),
        "Indices > 1 should match third condition"
    );
