...
pub fn auto_discover(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let middleware_mod = generate_mod_declarations(&src_dir, "middleware");
    let events_mod = generate_mod_declarations(&src_dir, "events");
    let definitions_mod = generate_mod_declarations(&src_dir, "definitions");

    // Conditionally generate embedded provider structs + registrations
    // Only when the embedded-views feature is active AND we're in a user project
    #[cfg(feature = "embedded-views")]
    let is_rustf_framework = {
        let cargo_toml_path = PathBuf::from(&manifest_dir).join("Cargo.toml");
    ...
    #[cfg(feature = "embedded-views")]
    let embedded_structs = if !is_rustf_framework {
        let views_folder = format!("{}/views/", manifest_dir);
        let assets_folder = format!("{}/public/", manifest_dir);
        quote! {
            // Auto-generated by #[auto_discover] — DO NOT EDIT
            // Embeds views/ and public/ from this crate at compile time.
            #[derive(rustf::rust_embed::RustEmbed)]
            #[folder = #views_folder]
            #[allow(dead_code)]
            struct __RustfAppViews;

            #[derive(rustf::rust_embed::RustEmbed)]
            #[folder = #assets_folder]
            #[allow(dead_code)]
            struct __RustfAppAssets;

            impl rustf::EmbeddedFileProvider for __RustfAppViews {
                fn get(&self, path: &str) -> Option<Vec<u8>> {
                    <__RustfAppViews as rustf::rust_embed::RustEmbed>::get(path)
                        .map(|f| f.data.into_owned())
                }
                fn files(&self) -> Vec<String> {
                    <__RustfAppViews as rustf::rust_embed::RustEmbed>::iter()
                        .map(|p| p.to_string())
                        .collect()
                }
            }

            impl rustf::EmbeddedFileProvider for __RustfAppAssets {
                fn get(&self, path: &str) -> Option<Vec<u8>> {
                    <__RustfAppAssets as rustf::rust_embed::RustEmbed>::get(path)
                        .map(|f| f.data.into_owned())
                }
                fn files(&self) -> Vec<String> {
                    <__RustfAppAssets as rustf::rust_embed::RustEmbed>::iter()
                        .map(|p| p.to_string())
                        .collect()
                }
            }
    ...
    let embedded_structs = quote! {};

    // Registration statements injected at the start of main()
    #[cfg(feature = "embedded-views")]
    let embed_register_stmt: syn::Stmt = if !is_rustf_framework {
        parse_quote! {
            {
                rustf::register_views_provider(Box::new(__RustfAppViews));
                rustf::register_assets_provider(Box::new(__RustfAppAssets));
            }
        }
    } else {
        parse_quote! { {} }
    ...
    #[cfg(not(feature = "embedded-views"))]
    let embed_register_stmt: syn::Stmt = parse_quote! { {} };
    ...
    let register_stmt = parse_quote! {
        rustf::auto::register_hooks(rustf::auto::AutoDiscoveryHooks {
            controllers: Some(|| controllers::collect_routes()),
            models: Some(|registry: &mut rustf::models::ModelRegistry| {
                models::register_all(registry);
            }),
    ...
    // Insert in reverse order so index 0 ends up first
    fn_block.stmts.insert(0, embed_register_stmt);
    ...
    let expanded = quote! {
        ...
        // Embedded file provider structs (only when embedded-views feature is active)
        #embedded_structs
...
fn generate_mod_declarations(src_dir: &PathBuf, dir_name: &str) -> proc_macro2::TokenStream {
    ...
    if !target_dir.exists() {
        return fallback_module(dir_name);
    }

    let mut generated_module_declarations = Vec::new();
    let mut generated_mod_content = String::new();
    generated_mod_content
        .push_str("// Auto-generated by RustF #[auto_discover] macro - DO NOT EDIT\n");
    generated_mod_content.push_str("// This file is framework-owned and may be overwritten.\n");
    generated_mod_content.push_str(&format!(
        "// Generated module declarations for src/{}/\n\n",
        dir_name
    ));
    let mut aggregator_calls = String::new();
        ...
        .filter_map(|e| e.ok())
    {
        ...
        if path_components.len() == 1 {
            ...
            generated_module_declarations.push(quote! {
                pub mod #module_ident;
            });
        } else {
        ...
        generated_mod_content.push_str(&format!("pub mod {};\n", module_ident_str));

        match dir_name {
            "controllers" => aggregator_calls.push_str(&format!(
                "    routes.extend({}::install());\n",
                module_ident_str
            )),
            "models" => aggregator_calls.push_str(&format!(
                "    {}::register(registry);\n",
                module_ident_str
            )),
            "middleware" => aggregator_calls.push_str(&format!(
                "    {}::install(registry);\n",
                module_ident_str
            )),
            "events" => aggregator_calls.push_str(&format!(
                "    {}::install(emitter);\n",
                module_ident_str
            )),
            "definitions" => aggregator_calls.push_str(&format!(
                "    {}::install(defs);\n",
                module_ident_str
            )),
            _ => {}
    ...
    if dir_name == "models" {
        ...
        if base_dir.exists() {
            ...
            if !base_declarations.is_empty() {
                ...
                generated_mod_content.push_str("\npub mod base {\n");
                generated_mod_content.push_str(&base_content);
                generated_mod_content.push_str("}\n");
            }
        }
    }
    ...
    match dir_name {
        "controllers" => generated_mod_content.push_str(
            "\npub fn collect_routes() -> Vec<rustf::routing::Route> {\n    let mut routes = Vec::new();\n",
        ),
        "models" => generated_mod_content.push_str(
            "\npub fn register_all(registry: &mut rustf::models::ModelRegistry) {\n",
        ),
        "middleware" => generated_mod_content.push_str(
            "\npub fn register_all(registry: &mut rustf::middleware::MiddlewareRegistry) {\n",
        ),
        "events" => generated_mod_content
            .push_str("\npub fn register_all(emitter: &mut rustf::events::EventEmitter) {\n"),
        "definitions" => generated_mod_content.push_str(
            "\npub fn register_all(defs: &mut rustf::definitions::Definitions) {\n",
        ),
        _ => {}
    }

    if !aggregator_calls.is_empty() {
        generated_mod_content.push_str(&aggregator_calls);
    }

    match dir_name {
        "controllers" => generated_mod_content.push_str("    routes\n}\n"),
        "models" | "middleware" | "events" | "definitions" => {
            generated_mod_content.push_str("}\n");
        }
        _ => {}
    }

    // Write generated support files
    if !generated_module_declarations.is_empty() {
        let legacy_ide_file_path = src_dir.join(format!("_{}.rs", dir_name));
        if legacy_ide_file_path.exists() {
            if let Err(e) = std::fs::remove_file(&legacy_ide_file_path) {
                let message = format!(
                    "RustF auto-discovery failed to remove legacy IDE file '{}': {}",
                    legacy_ide_file_path.display(),
                    e
                );
        ...
        let generated_dir = src_dir.join(".rustf");
        if let Err(e) = std::fs::create_dir_all(&generated_dir) {
            let message = format!(
                "RustF auto-discovery failed to create '{}': {}",
        ...
        let generated_file_path = generated_dir.join(format!("{}_gen.rs", dir_name));
        if let Err(e) = std::fs::write(&generated_file_path, &generated_mod_content) {
            let message = format!(
                "RustF auto-discovery failed to write '{}': {}",
        ...
        if let Err(message) = ensure_wrapper_module(src_dir, dir_name) {
            return quote! { compile_error!(#message); };
        }
    }
    ...
    // Return wrapper module import for compilation
    let dir_ident = syn::Ident::new(dir_name, proc_macro2::Span::call_site());
    let wrapper_relative_path = format!("{}/mod.rs", dir_name);
    quote! {
...
fn fallback_module(dir_name: &str) -> proc_macro2::TokenStream {
    let dir_ident = syn::Ident::new(dir_name, proc_macro2::Span::call_site());
    match dir_name {
        "controllers" => quote! {
            mod #dir_ident {
        ...
        "models" => quote! {
            mod #dir_ident {
                pub fn register_all(_registry: &mut rustf::models::ModelRegistry) {}
            }
        },
        "middleware" => quote! {
            mod #dir_ident {
                pub fn register_all(_registry: &mut rustf::middleware::MiddlewareRegistry) {}
            }
        },
        "events" => quote! {
            mod #dir_ident {
                pub fn register_all(_emitter: &mut rustf::events::EventEmitter) {}
            }
        },
        "definitions" => quote! {
            mod #dir_ident {
                pub fn register_all(_defs: &mut rustf::definitions::Definitions) {}
            }
        },
        _ => quote! {
            mod #dir_ident {}
...
fn ensure_wrapper_module(src_dir: &PathBuf, dir_name: &str) -> Result<(), String> {
    let wrapper_path = src_dir.join(dir_name).join("mod.rs");
    if !wrapper_path.exists() {
        ...
        std::fs::write(&wrapper_path, wrapper_template(dir_name)).map_err(|e| {
            format!(
                "RustF auto-discovery failed to write '{}': {}",
    ...
    validate_wrapper_module(&wrapper_path, dir_name)
}

fn wrapper_template(dir_name: &str) -> String {
    format!(
        "// Auto-generated scaffold for RustF auto-discovery.\n\
// You may add custom code here, but keep the generated import and re-export intact.\n\
#[path = \"../.rustf/{dir_name}_gen.rs\"]\n\
mod generated;\n\n\
pub use generated::*;\n"
    )
}

fn validate_wrapper_module(wrapper_path: &std::path::Path, dir_name: &str) -> Result<(), String> {
    let expected_path = format!("../.rustf/{}_gen.rs", dir_name);
    let contents = std::fs::read_to_string(wrapper_path).map_err(|e| {
        format!(
            "RustF auto-discovery failed to read '{}': {}",
            wrapper_path.display(),
            e
        )
    })?;

    let parsed = syn::parse_file(&contents).map_err(|e| {
        format!(
            "RustF auto-discovery found invalid Rust in '{}': {}",
            wrapper_path.display(),
            e
        )
    })?;

    let mut has_generated_mod = false;
    let mut has_generated_reexport = false;

    for item in parsed.items {
        match item {
            syn::Item::Mod(item_mod) if item_mod.ident == "generated" => {
                if item_mod.content.is_some() {
                    return Err(format!(
                        "RustF auto-discovery rejected '{}': `mod generated` must stay external and point to '{}'",
                        wrapper_path.display(),
                        expected_path
                    ));
                }

                let mut matches_expected_path = false;
                for attr in item_mod.attrs {
                    if attr.path().is_ident("path") {
                        if let syn::Meta::NameValue(name_value) = &attr.meta {
                            if let syn::Expr::Lit(expr_lit) = &name_value.value {
                                if let syn::Lit::Str(lit) = &expr_lit.lit {
                                    matches_expected_path = lit.value() == expected_path;
                                }
                            }
                        }
                    }
                ...
                if !matches_expected_path {
                    return Err(format!(
                        "RustF auto-discovery rejected '{}': `mod generated` must use #[path = \"{}\"]",
                        wrapper_path.display(),
                        expected_path
                    ));
                }

                has_generated_mod = true;
            }
            syn::Item::Use(item_use) if is_generated_glob_reexport(&item_use) => {
                has_generated_reexport = true;
            }
            _ => {}
        }
    }

    if !has_generated_mod {
        return Err(format!(
            "RustF auto-discovery rejected '{}': missing required generated module import `#[path = \"{}\"] mod generated;`",
            wrapper_path.display(),
            expected_path
        ));
    }

    if !has_generated_reexport {
        return Err(format!(
            "RustF auto-discovery rejected '{}': missing required `pub use generated::*;` re-export",
            wrapper_path.display()
        ));
    }
...
fn is_generated_glob_reexport(item_use: &syn::ItemUse) -> bool {
    if !matches!(item_use.vis, syn::Visibility::Public(_)) {
        return false;
    }

    match &item_use.tree {
        syn::UseTree::Path(path) if path.ident == "generated" => {
...
mod tests {
    ...
    #[test]
    fn generate_mod_declarations_creates_wrapper_and_generated_files() {
        let temp_dir = create_manifest("fixture-app");
        write_rs(
            &temp_dir.path().join("src/controllers/home.rs"),
            "pub fn install() -> Vec<()> { vec![] }",
        ...
        let _guard = ManifestGuard::enter(temp_dir.path());
        let tokens = generate_mod_declarations(&temp_dir.path().join("src"), "controllers")
            .to_string();

        assert!(tokens.contains("# [path = \"controllers/mod.rs\"] mod controllers ;"));

        let wrapper_path = temp_dir.path().join("src/controllers/mod.rs");
        let generated_path = temp_dir.path().join("src/.rustf/controllers_gen.rs");

        let wrapper = fs::read_to_string(wrapper_path).expect("wrapper content");
        let generated = fs::read_to_string(generated_path).expect("generated content");

        assert!(wrapper.contains("#[path = \"../.rustf/controllers_gen.rs\"]"));
        assert!(wrapper.contains("mod generated;"));
        assert!(wrapper.contains("pub use generated::*;"));

        assert!(generated.contains("#[path = \"../controllers/home.rs\"]"));
        assert!(generated.contains("pub mod home;"));
        assert!(!temp_dir.path().join("src/_controllers.rs").exists());
    }

    #[test]
    fn generate_mod_declarations_removes_legacy_underscore_file() {
        let temp_dir = create_manifest("fixture-app");
        write_rs(
            &temp_dir.path().join("src/models/users.rs"),
            "pub fn register(_: &mut ()) {}",
        );
        write_rs(
            &temp_dir.path().join("src/_models.rs"),
            "// legacy auto-generated IDE file",
        );

        let _guard = ManifestGuard::enter(temp_dir.path());
        let tokens = generate_mod_declarations(&temp_dir.path().join("src"), "models").to_string();

        assert!(tokens.contains("# [path = \"models/mod.rs\"] mod models ;"));
        assert!(!temp_dir.path().join("src/_models.rs").exists());
    }

    #[test]
    fn generate_mod_declarations_rejects_invalid_wrapper_shape() {
        let temp_dir = create_manifest("fixture-app");
        write_rs(
            &temp_dir.path().join("src/controllers/home.rs"),
            "pub fn install() -> Vec<()> { vec![] }",
        ...
        assert!(tokens.contains("mod generated"));
        assert!(tokens.contains("controllers_gen.rs"));
    }

    #[test]
    fn generate_mod_declarations_accepts_valid_existing_wrapper() {
        let temp_dir = create_manifest("fixture-app");
        write_rs(
            &temp_dir.path().join("src/models/users.rs"),
            "pub fn register(_: &mut ()) {}",
        ...
        write_rs(
            &temp_dir.path().join("src/models/mod.rs"),
            "// custom comment is allowed\n#[path = \"../.rustf/models_gen.rs\"]\nmod generated;\n\npub use generated::*;\n",
        );
