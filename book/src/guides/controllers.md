...
- `Ok(BeforeAction::Continue)` — the handler runs as usual.
- `Ok(BeforeAction::Stop)` — the handler is skipped. The framework
  returns whatever response the hook wrote to `ctx.res` via
  `ctx.redirect`, `ctx.throw403`, `ctx.json`, etc.
