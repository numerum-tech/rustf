//! SQLite-specific schema management implementation for RustF CLI
...
fn prepare_base_model_variables(
    ...
    vars.insert("pool_getter".to_string(), "sqlite_pool".to_string()); // SQLite specific pool getter
    ...
    for field in &sorted_fields {
        ...
        let rust_type = if let Some(ref lang_type) = field.lang_type {
            ...
            match field.field_type.base_type() {
                // SQLite has limited type system, but we preserve semantic types
                // SQLite stores all integers as INTEGER (up to 64-bit)
                "tinyint" => {
                    if is_nullable {
                        "Option<i8>"
                    } else {
                ...
                // Unsigned integers (SQLite doesn't distinguish, but we preserve types)
                "unsigned_tinyint" => {
                    if is_nullable {
                        "Option<u8>"
                    } else {
                ...
                // Text types (SQLite stores all as TEXT)
                "varchar" | "char" | "string" => {
                    if is_nullable {
                        "Option<String>"
                    } else {
                ...
                // Date/Time types (SQLite stores as TEXT or REAL)
                "timestamp" | "datetime" => {
                    if is_nullable {
                        "Option<DateTime<Utc>>"
                    } else {
                ...
                // Semantic types (SQLite stores as TEXT)
                "json" => {
                    if is_nullable {
    ...
    for field in &sorted_fields {
        if let rustf_schema::types::FieldType::Enum { values, .. } = &field.field_type {
            ...
            // Generate field-specific enum converter method (pass-through for SQLite)
            // SQLite doesn't need type casting, but we provide the method for API consistency
            enum_converters.push(format!(
                "    /// Convert a value to SQLite enum format for {} field\n    /// \n    /// # Example\n    /// ```\n    /// let value = {}::as_{}_enum(\"ACTIVE\");\n    /// // Returns: \"ACTIVE\" (pass-through for SQLite)\n    /// ```\n    pub fn as_{}_enum(value: &str) -> String {{\n        value.to_string()\n    }}",
