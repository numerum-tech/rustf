...
pub async fn generate_controller(names: String, crud: bool, routes: bool) -> Result<()> {
    ...
    for name in controller_names {
        ...
        vars.insert("description", format!("{} operations", to_title_case(&snake_name)));
...
pub async fn generate_module(name: String, shared: bool, with_methods: bool) -> Result<()> {
    ...
    vars.insert("description", format!("{} functionality", to_title_case(&snake_name)));
...
pub async fn generate_middleware(name: String, auth: bool, logging: bool, priority: i32) -> Result<()> {
    let project_path = std::env::current_dir()?;
    ...
    vars.insert("description", format!("{} middleware for request processing", to_title_case(&snake_name)));
    ...
    println!("   - Priority {} means: {}", 
        priority,
        if priority < 0 { "runs early in the chain" } 
        else if priority > 0 { "runs late in the chain" } 
        else { "default execution order" }
    );
...
pub async fn generate_event(name: String, lifecycle: bool, custom: bool) -> Result<()> {
    ...
    vars.insert("description", format!("{} events", to_title_case(&snake_name)));
    ...
    if custom {
        ...
        println!("   - Example: ctx.emit(\"{}.data.received\", data)?;", snake_name);
    }
