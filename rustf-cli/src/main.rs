...
async fn main() -> anyhow::Result<()> {
    ...
    let project_path = cli.project.unwrap_or_else(|| std::env::current_dir().unwrap());
    ...
    match cli.command {
        Commands::Analyze { command } => {
            ...
            match command {
                ...
                AnalyzeCommand::Controllers { name } => {
                    controllers::run(project_path, name).await
                }
                AnalyzeCommand::Discover { filter } => {
                    discover::run(project_path, filter).await
                }
                ...
                AnalyzeCommand::Routes { conflicts_only, validate } => {
                    commands::routes::run(project_path, conflicts_only, validate).await
                }
                AnalyzeCommand::Views { layout, name, security } => {
                    commands::views::run(project_path, name, layout, security).await
                }
            }
        }
        Commands::Db(db_cmd) => {
            db_cmd.execute(project_path).await
        }
        Commands::Export { format, include_code, output } => {
        ...
        Commands::Perf { command } => {
            ...
            match command {
                ...
                PerfCommand::CacheStats {} => {
                    commands::cache_stats::run(project_path).await
                }
                PerfCommand::Stream { aggressive_cleanup, chunk_size, format, max_concurrent, memory_limit } => {
                    commands::stream::run(project_path, memory_limit, chunk_size, max_concurrent, aggressive_cleanup, format).await
                }
            }
        }
        Commands::Query { item_type, item_name, format } => {
            match item_type {
                ...
                _ => {
                    commands::query::run(project_path, format!("{:?}", item_type).to_lowercase(), item_name).await
                }
            }
        }
        Commands::Schema(schema_cmd) => {
            schema_cmd.execute().await
        }
        Commands::Translations { command } => {
            commands::translations::execute(&command)
        }
        Commands::Serve { command } => {
            use commands::serve_cmd::ServeCommand;
            match command {
                ServeCommand::Start { allow_writes, auto_port, bind, name, port, watch, websocket } => {
                    let final_port = if auto_port {
                        match serve::find_available_port(port).await {
                            ...
                            None => {
                                anyhow::bail!("Could not find an available port starting from {}", port);
                            }
                        }
                    } else {
                        port
                    };
                    serve::run(project_path, final_port, bind, watch, websocket, name, allow_writes).await
                }
                ServeCommand::List {} => {
                    serve::list().await
                }
                ServeCommand::Stop { port } => {
                    serve::stop(port).await
                }
            }
        }
