...
use super::{AutoGenerate, Field, FieldType, Result, Schema, SchemaError, Table, ValidationResult};
...
impl SchemaValidator {
    ...
    fn validate_field(table_name: &str, field_name: &str, field: &Field) -> Result<()> {
        ...
        if let Some(auto) = &field.constraints.auto {
            match auto {
                AutoGenerate::Boolean(true) => {
                    ...
                    {
                        return Err(SchemaError::Validation(format!(
                            "Field '{}.{}' with auto=true must be primary key or serial/uuid type",
                            table_name, field_name
                        )));
                    }
                }
                ...
                AutoGenerate::Type(t) => match t.as_str() {
                    "create" | "update" => {
                        if !matches!(field.field_type.base_type(), "timestamp" | "datetime") {
                            return Err(SchemaError::Validation(format!(
                                "Field '{}.{}' with auto='{}' must be timestamp type",
        ...
        if let Some(fk) = &field.constraints.foreign_key {
            if !fk.contains('.') {
                return Err(SchemaError::Validation(format!(
                    "Foreign key '{}' in field '{}.{}' must be in format 'table.field'",
    ...
    fn validate_field_comprehensive(
        table_name: &str,
        field_name: &str,
        field: &Field,
    ) -> ValidationResult {
        ...
        match field_type {
            FieldType::Simple(type_name) => {
                ...
                match type_name.as_str() {
                    "int" | "integer" | "serial" | "bigint" | "smallint" | "tinyint"
                    | "mediumint" | "string" | "text" | "tinytext" | "mediumtext" | "longtext"
                    | "varchar" | "decimal" | "float" | "double" | "boolean" | "bool"
                    | "timestamp" | "datetime" | "date" | "time" | "json" | "jsonb" | "uuid"
                    | "blob" | "enum" | "inet" | "cidr" => Ok(()),
            ...
            FieldType::Parameterized { base_type, params } => {
                ...
                match base_type.as_str() {
                    ...
                    "decimal" | "numeric" => {
                        if params.len() != 2 {
                            return Err(SchemaError::InvalidType(format!(
                                "Type '{}' requires exactly 2 parameters (precision, scale)",
                                base_type
                            )));
                        }
                    }
                ...
                if let Some(trans) = transitions {
                    for (from, to_states) in trans {
                        if !values.contains(from) {
                            return Err(SchemaError::InvalidType(format!(
                                "Transition from state '{}' not in enum values",
                                from
                            )));
                        }
                        for to in to_states {
                            if !values.contains(to) {
                                return Err(SchemaError::InvalidType(format!(
                                    "Transition to state '{}' not in enum values",
                                    to
                                )));
                            }
                        }
    ...
    fn validate_foreign_keys(schema: &Schema) -> Result<()> {
        for (table_name, table) in &schema.tables {
            for (field_name, field) in &table.fields {
                if let Some(fk) = &field.constraints.foreign_key {
                    let (_target_table, target_field) =
                        schema.resolve_field_ref(fk).map_err(|_| {
                            SchemaError::Validation(format!(
                                "Foreign key '{}' in '{}.{}' references non-existent field",
                                fk, table_name, field_name
                            ))
                        })?;
            ...
            if let Some(belongs_to) = &table.relations.belongs_to {
                for (relation_name, relation) in belongs_to {
                    ...
                    if !schema.tables.contains_key(&relation.model) {
                        return Err(SchemaError::Validation(format!(
                            "Relation '{}.{}' references non-existent model '{}'",
                    ...
                    if !table.fields.contains_key(&relation.local_field) {
                        return Err(SchemaError::Validation(format!(
                            "Relation '{}.{}' references non-existent local field '{}'",
                    ...
                    if !target_table.fields.contains_key(&relation.foreign_field) {
                        return Err(SchemaError::Validation(format!(
                            "Relation '{}.{}' references non-existent foreign field '{}.{}'",
        ...
        for (table_name, table) in &schema.tables {
            ...
            if let Some(has_many) = &table.relations.has_many {
                for (relation_name, relation) in has_many {
                    ...
                    if !schema.tables.contains_key(&relation.model) {
                        result.add_error(format!(
                            "HasMany relation '{}.{}' references non-existent model '{}'",
                            table_name, relation_name, relation.model
                        ));
                    } else {
                        // Check that local field exists
                        if !table.fields.contains_key(&relation.local_field) {
                            result.add_error(format!(
                                "HasMany relation '{}.{}' references non-existent local field '{}'",
                                table_name, relation_name, relation.local_field
                            ));
                        }
