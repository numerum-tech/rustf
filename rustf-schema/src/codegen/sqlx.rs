...
use crate::codegen::{CodeGenerator, GenerationContext, TemplateGenerator};
use crate::{FieldType, Result, Schema, Table};
...
impl SqlxGenerator {
    ...
    pub fn new() -> Result<Self> {
        ...
        template_generator
            .register_template("relations", include_str!("templates/sqlx_relations.hbs"))?;
    ...
    pub fn field_type_to_rust(&self, field_type: &FieldType, nullable: bool) -> String {
        let base_type = match field_type {
            FieldType::Simple(t) => match t.as_str() {
                "int" | "integer" => "i32",
                "bigint" => "i64",
                "serial" => "i32",
            ...
            FieldType::Parameterized { base_type, .. } => match base_type.as_str() {
    ...
    pub fn field_type_to_sqlx(&self, field_type: &FieldType) -> String {
        match field_type {
            ...
            FieldType::Parameterized { base_type, .. } => base_type.to_uppercase(),
    ...
    fn generate_rust_fields(&self, table: &Table) -> Result<Vec<RustField>> {
        ...
        for (field_name, field) in &table.fields {
            let rust_type = self.field_type_to_rust(
                &field.field_type,
                field.constraints.nullable.unwrap_or(false),
            );
            ...
            fields.push(RustField {
                ...
                auto_increment: matches!(
                    ...
                    Some(crate::types::AutoGenerate::Boolean(true))
                ),
    ...
    fn needs_decimal_import(&self, rust_fields: &[RustField]) -> bool {
        rust_fields.iter().any(|field| {
            field.rust_type.contains("rust_decimal::Decimal") || field.rust_type.contains("Decimal")
        })
    }
    ...
    fn needs_uuid_import(&self, rust_fields: &[RustField]) -> bool {
        rust_fields
            .iter()
            .any(|field| field.rust_type.contains("uuid::Uuid") || field.rust_type.contains("Uuid"))
    }

    /// Check if JSON import is needed in Types module
    fn needs_json_import(&self, rust_fields: &[RustField]) -> bool {
        rust_fields.iter().any(|field| {
            field.rust_type.contains("serde_json::Value") || field.rust_type.contains("Value")
        })
    }
}
