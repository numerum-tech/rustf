...
impl SchemaParser {
    ...
    pub async fn parse_file(path: &Path) -> Result<HashMap<String, Table>> {
        ...
        let tables: HashMap<String, Table> = serde_yaml::from_str(&contents).map_err(|e| {
    ...
    pub fn parse_directory_sync(dir: &Path) -> Result<Schema> {
        ...
        if meta_path.exists() {
            ...
            schema.meta = Some(serde_yaml::from_str(&meta_content).map_err(|e| {
                SchemaError::Parse(format!(
                    "Failed to parse meta file '{}': {}",
                    meta_path.display(),
                    e
                ))
            })?);
    ...
    pub async fn parse_directory(dir: &Path) -> Result<Schema> {
        ...
        if let Some(meta_path) = meta_path {
            ...
            schema.meta = Some(
                serde_yaml::from_str::<SchemaMeta>(&meta_content).map_err(|e| {
                    SchemaError::Parse(format!(
                        "Failed to parse meta file '{}': {}",
                        meta_path.display(),
                        e
