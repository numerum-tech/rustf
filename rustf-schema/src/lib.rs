...
impl Schema {
    ...
    pub fn validate_consistency(
        &self,
        generated_checksums: &HashMap<String, String>,
    ) -> Result<()> {
        let schema_checksum = self.checksum();

        for (table_name, generated_checksum) in generated_checksums {
            if !self.tables.contains_key(table_name) {
                return Err(SchemaError::Consistency(format!(
                    "Generated code exists for non-existent table '{}'",
                    table_name
                )));
            }
        ...
        for table_name in self.tables.keys() {
            if !generated_checksums.contains_key(table_name) {
                return Err(SchemaError::Consistency(format!(
                    "No generated code found for table '{}'. Run code generation.",
                    table_name
                )));
            }
        }
